...
def test_complete_data_flow():
    ...
    try:
        ...
        from agents.tools import get_company_data, analyze_company_profile, validate_ticker
        ...
        if interpretation.companies and len(interpretation.companies) > 0:
            ...
            if validation_result.get("valid", False):
                ...
                print(f"  📊 Company data retrieved: {company_data_result.get('success', False)}")
                
                if company_data_result.get("success", False):
                    state["financial_data"] = {ticker: company_data_result.get("data", {})}
                    ...
                    if analysis_result.get("success", False):
                        state["analysis_results"] = {ticker: analysis_result.get("analysis", {})}
...
def test_all_analysis_types():
    ...
    try:
        ...
        from agents.tools import (
            analyze_company_profile, analyze_financial_metrics,
            analyze_financial_statements, compare_companies
        )
        ...
        for test_case in test_cases:
            ...
            if type_match and has_companies:
                ...
                try:
                    ...
                    valid_tickers = [t for t in interpretation.companies 
                                   if len(t) >= 3 and len(t) <= 5 and t.isalpha() and t.isupper() 
                    ...
                    if test_case['expected_type'] == 'comparison':
                        ...
                        if len(valid_tickers) >= 2:
                            ...
                            result = {"success": False, "error": "Insufficient valid tickers"}
                        ...
                        if valid_tickers:
                            ...
                            result = {"success": False, "error": "No valid ticker found"}
                    else:
                        if valid_tickers:
                            result = test_case['tool'].invoke({
                                "ticker": valid_tickers[0]
                    ...
                    if result.get("success", False):
                        ...
                        print(f"  ⚠️  Tool execution failed: {result.get('error', 'Unknown error')}")
                        
                except Exception as e:
                    print(f"  ❌ Tool execution error: {e}")
            else:
                print(f"  ❌ Interpretation failed - Type: {type_match}, Companies: {has_companies}")
        ...
        for field, expected_type in type_checks:
            if field in state:
                if not isinstance(state[field], expected_type):
                    type_errors.append(f"{field}: expected {expected_type}, got {type(state[field])}")
...
def test_analysis_quality_regression():
    ...
    try:
        ...
        from agents.tools import analyze_company_profile, get_company_data, validate_ticker
        ...
        if validation.get("valid", False):
            ...
            if data_result.get("success", False):
                analysis_result = analyze_company_profile.invoke({
                    "ticker": ticker,
                    "company_data": data_result.get("data", {})
                })
                    ...
                    for indicator, expected_type, data_source in quality_indicators:
                        if indicator in data_source:
                            ...
                            if isinstance(value, expected_type) and value != 'N/A':
                                ...
                                print(f"    ⚠️  {indicator}: {value} (type: {type(value)})")
                        else:
                            print(f"    ❌ {indicator}: missing")
                    
                    quality_percentage = (quality_score / len(quality_indicators)) * 100
                    print(f"  📊 Analysis Quality: {quality_percentage:.1f}% ({quality_score}/{len(quality_indicators)})")
