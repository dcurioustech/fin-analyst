...
# Import utilities
from utils.error_handling import setup_logging, log_operation_start, log_operation_end
...
class FinancialAnalysisApp:
    ...
    def _fetch_company_data(self, ticker: str) -> Optional[dict]:
        ...
        try:
            ...
            if result['success']:
                ...
                log_operation_end("fetch_company_data", False, ticker=ticker, error=result['error'])
                return None
    ...
    def _handle_company_profile(self, company_data: dict) -> None:
        ...
        try:
            ...
            formatted_output = self.display_formatter.format_company_profile(profile_result)
            print(formatted_output)
            
            log_operation_end("company_profile_analysis", profile_result.get('success', False))
    ...
    def _handle_key_metrics(self, company_data: dict) -> None:
        ...
        try:
            ...
            metrics_result = self.metrics_analyzer.get_comprehensive_metrics(company_data)
            
            # Format and display results
            formatted_output = self.display_formatter.format_metrics_display(metrics_result)
            print(formatted_output)
            
            log_operation_end("key_metrics_analysis", metrics_result.get('success', False))
    ...
    def _handle_financial_statement(self, ticker: str, statement_type: str, statement_name: str) -> None:
        """
        ...
        try:
            ...
            if statement_type == 'income_statement':
                analysis_result = self.statement_analyzer.process_income_statement(statement_data)
            elif statement_type == 'balance_sheet':
                analysis_result = self.statement_analyzer.process_balance_sheet(statement_data)
            elif statement_type == 'cash_flow':
                analysis_result = self.statement_analyzer.process_cash_flow(statement_data)
            else:
                print(f"Unknown statement type: {statement_type}")
                return
            
            # Format and display results
            formatted_output = self.display_formatter.format_statement_display(analysis_result, statement_name)
            print(formatted_output)
            
            log_operation_end(f"{statement_type}_analysis", analysis_result.get('success', False), ticker=ticker)
    ...
    def _handle_recommendations(self, ticker: str) -> None:
        ...
        try:
            ...
            # Format and display results
            formatted_output = self.display_formatter.format_recommendations_display(recommendations_result)
            print(formatted_output)
            
            log_operation_end("recommendations_analysis", recommendations_result.get('success', False), ticker=ticker)
    ...
    def _handle_peer_comparison(self, ticker: str) -> None:
        ...
        try:
            # Get suggested peers
            suggested_peers = self.data_service.suggest_peers_from_recommendations(ticker)
            ...
            comparison_data_result = self.data_service.get_peer_comparison_data(ticker, peer_tickers)
            
            if not comparison_data_result['success']:
                print(f"Error: {comparison_data_result.get('error', 'Failed to fetch comparison data')}")
                return
            ...
            # Format and display results
            formatted_output = self.display_formatter.format_comparison_display(comparison_result)
            print(formatted_output)
            
            log_operation_end("peer_comparison_analysis", comparison_result.get('success', False), ticker=ticker)
