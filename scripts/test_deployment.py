...
def test_web_interface(base_url: str) -> bool:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if "Financial Analysis Assistant" in content and "chat-container" in content:
...
def run_comprehensive_test(base_url: Optional[str] = None) -> Dict[str, bool]:
    ...
    if base_url:
        ...
        if results['chat_api']:
            ...
            results['chat_comparison'] = test_chat_endpoint(base_url, "Compare Apple and Microsoft")
            results['chat_metrics'] = test_chat_endpoint(base_url, "Show me Tesla's financial metrics")
...
def main():
    """Main test function."""
    parser = argparse.ArgumentParser(description="Test Financial Analysis Assistant deployment")
    parser.add_argument("--url", help="Base URL to test (e.g., https://your-service-url)")
    parser.add_argument("--local-only", action="store_true", help="Test only local functionality")
    parser.add_argument("--message", default="Analyze Apple", help="Test message for chat API")
