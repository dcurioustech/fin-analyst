    ...
    try:
        ...
        for case in invalid_cases:
            ...
            if invalid_tickers and not valid_tickers:
                ...
                print(
                    f"  🔄 Recovery: Proceed with valid tickers, warn about invalid ones"
                )
            else:
...
def test_unclear_request_clarification():
    ...
    try:
        ...
        for case in unclear_cases:
            ...
            expected_unclear = (
                not interpretation.companies
                or interpretation.confidence < 0.5
                or interpretation.analysis_type is None
            )
...
def test_system_error_graceful_degradation():
    ...
    try:
        ...
        try:
            ...
            updated_state = update_state_with_user_input(
                corrupted_state, "Analyze AAPL"
            )
    ...
    try:
        ...
        for i, case in enumerate(edge_cases, 1):
            ...
            try:
                ...
                print(
                    f"  📊 Companies: {interpretation.companies[:3]}{'...' if len(interpretation.companies) > 3 else ''}"
                )
                print(f"  🔍 Analysis type: {interpretation.analysis_type}")
                print(f"  📈 Confidence: {interpretation.confidence:.2f}")
                print(f"  ❓ Needs clarification: {interpretation.needs_clarification}")
...
def main():
    ...
    print("=" * 35)
    print(
        f"Invalid Ticker Recovery: {'✅ PASS' if ticker_recovery_success else '❌ FAIL'}"
    )
    print(
        f"Unclear Request Clarification: {'✅ PASS' if clarification_success else '❌ FAIL'}"
    )
    print(
        f"System Error Degradation: {'✅ PASS' if degradation_success else '❌ FAIL'}"
    )
