    ...
    if result['success']:
        ...
        if competitive_analysis['success']:
            ...
            for metric, ranking in positioning.get('rankings', {}).items():
                percentile = positioning.get('percentile_rankings', {}).get(metric, 'N/A')
            ...
            for column in df_numeric.columns:
                if 'SMCI' in df_numeric.index:
                    ...
                    if higher_is_better:
                        ...
                        percentile = ((column_values <= smci_value).sum() / len(column_values)) * 100
                    else:
                        rank = (column_values < smci_value).sum() + 1
                        percentile = ((column_values >= smci_value).sum() / len(column_values)) * 100
        ...
        pe_weak = any('p/e' in weakness.lower() for weakness in positioning.get('weaknesses', []))
        ps_weak = any('p/s' in weakness.lower() for weakness in positioning.get('weaknesses', []))
        pb_weak = any('p/b' in weakness.lower() for weakness in positioning.get('weaknesses', []))
        ...
        if pe_weak and ps_weak and pb_weak:
            print("✅ FIX VERIFIED: SMCI correctly identified as having weak valuation ratios")
        else:
            print("❌ FIX NOT WORKING: SMCI should have weak valuation ratios compared to NVDA")
