...
def test_without_langgraph():
    ...
    try:
        ...
        for input_text, expected_companies, expected_type in test_cases:
            ...
            print(f"  🔤 '{input_text}'")
            print(
                f"    📊 Companies: {interpretation.companies} (expected: {expected_companies})"
            )
            print(
                f"    🔍 Type: {interpretation.analysis_type} (expected: {expected_type})"
            )
        ...
        clarification = default_response_generator.generate_clarification_request(
            "unclear request"
        )
...
def test_with_mock_orchestrator():
    ...
    try:
        ...
        def mock_orchestrator_process(user_input: str, state=None):
            ...
            if interpretation.needs_clarification:
                ...
                if interpretation.analysis_type == "comparison":
                    ...
                    response = (
                        f"Here's the analysis for {interpretation.companies[0]}..."
                    )
            else:
        ...
        for test_case in test_cases:
            ...
            # Validate results
            companies_match = set(result_state.get("companies", [])) == set(
            ...
            if companies_match and type_match and has_response:
                ...
                print(
                    f"❌ FAIL - Companies: {companies_match}, Type: {type_match}, Response: {has_response}"
                )

        # Test follow-up context
        print(f"\n📋 Test: Follow-up Context Maintenance")
        initial_state = mock_orchestrator_process("Analyze AAPL")
        followup_state = mock_orchestrator_process(
            "What about its revenue?", initial_state
        )

        print(f"📊 Context maintained: {followup_state.get('companies', [])}")
        print(
            f"💬 Follow-up response: {followup_state.get('agent_response', '')[:100]}..."
        )

        context_maintained = followup_state.get("companies") == initial_state.get(
            "companies"
        )
...
def test_error_scenarios():
    ...
    try:
        ...
        for request in malformed_requests:
            ...
            print(
                f"  🔤 '{request}': needs_clarification={interpretation.needs_clarification}"
            )
        ...
        clarification_response = (
            default_response_generator.generate_clarification_request(
                "I couldn't understand your request. Please try again."
            )
        )
        print(
            f"  ✅ Error handling via clarification: {len(clarification_response)} characters"
        )
