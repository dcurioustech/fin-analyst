...
from utils.error_handling import validate_ticker_symbol, RetryHandler
...
class MenuHandler:
    ...
    def get_user_choice(self, options: List[str], prompt: str = "Please enter your choice: ") -> str:
        """
    ...
    def get_ticker_input(self, prompt: str = "Enter a stock ticker (e.g., AAPL, GOOGL) or type 'exit' to quit: ") -> str:
        """
        ...
        while attempt < max_attempts:
            try:
                ...
                if not validate_ticker_symbol(ticker):
                    print("Invalid ticker format. Ticker should be 1-10 letters only (e.g., AAPL, MSFT).")
                    attempt += 1
                    continue
    ...
    def get_peer_tickers_input(self, suggested_peers: List[str] = None) -> List[str]:
        ...
        try:
            ...
            peer_input = input("Enter peer tickers separated by spaces (or press Enter to use suggestions): ").strip()
            
            if peer_input:
                # Parse and validate user input
                raw_peers = [ticker.strip().upper() for ticker in peer_input.split() if ticker.strip()]
                valid_peers = []
                invalid_peers = []
                ...
                if invalid_peers:
                    print(f"Warning: Invalid ticker symbols ignored: {', '.join(invalid_peers)}")
                ...
                if len(valid_peers) > 10:
                    print(f"Too many peers provided. Using first 10: {', '.join(valid_peers[:10])}")
                ...
                valid_suggested = [p for p in suggested_peers if validate_ticker_symbol(p)]
    ...
    def validate_menu_choice(self, choice: str, valid_choices: List[str] = None) -> bool:
        """
    ...
    def handle_ticker_validation_error(self, ticker: str) -> None:
        ...
        """
        print(f"Error: Could not find data for ticker '{ticker}'. Please check the symbol and try again.")
