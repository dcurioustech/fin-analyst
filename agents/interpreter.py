...
class RuleBasedInterpreter(BaseInterpreter):
    ...
    def interpret_request(
        ...
        interpretation.analysis_type = self._determine_analysis_type(
            user_input_lower, interpretation.companies
        )
        ...
        logger.info(
            f"Interpreted request - Companies: {interpretation.companies}, "
    ...
    def _extract_companies(
        ...
        if not companies and context:
            ...
            if context_companies and any(
                word in user_input_lower
                for word in ["it", "them", "they", "this", "that"]
            ):
    ...
    def _calculate_confidence(self, interpretation: RequestInterpretation) -> float:
        ...
        if interpretation.analysis_type in [
            "income_statement",
            "balance_sheet",
            "cash_flow",
        ]:
...
class HybridInterpreter(BaseInterpreter):
    ...
    def __init__(self, llm_client=None, confidence_threshold: float = 0.7):
        ...
        logger.info(
            f"Hybrid interpreter initialized (threshold: {confidence_threshold})"
        )
    ...
    def interpret_request(
        ...
        if (
            ...
            and rule_interpretation.confidence < self.confidence_threshold
        ):
            logger.info("Attempting LLM interpretation (low confidence)")
            llm_interpretation = self.llm_interpreter.interpret_request(
                user_input, context
            )
