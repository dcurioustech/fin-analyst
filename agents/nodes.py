...
from .response_generator import default_response_generator, ResponseContext
...
def orchestrator_entry_node(state: FinancialOrchestratorState) -> FinancialOrchestratorState:
    """
    ...
    """
    logger.info(f"Orchestrator entry - processing user input: {state['user_input'][:100]}...")
...
def interpreter_routing_node(state: FinancialOrchestratorState) -> FinancialOrchestratorState:
    """
    ...
    try:
        ...
        interpretation = default_interpreter.interpret_request(state["user_input"], context)
        ...
        logger.info(f"Interpretation complete - Companies: {state['companies']}, "
...
def analysis_planning_node(state: FinancialOrchestratorState) -> FinancialOrchestratorState:
    """
    ...
    try:
        ...
        if analysis_type == "comparison" and len(companies) > 1:
            ...
            plan["tools"] = ["get_financial_statements_data", "analyze_financial_statements"]
        ...
        logger.info(f"Analysis plan created - Tools: {plan['tools']}, "
...
def data_collection_node(state: FinancialOrchestratorState) -> FinancialOrchestratorState:
    """
            ...
            if requirement == "company_data":
                for ticker in companies:
                    ...
                    if result.get("success"):
                        ...
                        logger.warning(f"Failed to get company data for {ticker}: {result.get('error')}")
                ...
                for ticker in companies:
                    ...
                    if result.get("success"):
                        ...
                        logger.warning(f"Failed to get statements data for {ticker}: {result.get('error')}")
                ...
                result = tool.invoke({"main_ticker": main_ticker, "peer_tickers": peer_tickers})
                if result.get("success"):
                    state["financial_data"]["comparison_data"] = result["data"]
                else:
                    logger.warning(f"Failed to get comparison data: {result.get('error')}")
...
def analysis_execution_node(state: FinancialOrchestratorState) -> FinancialOrchestratorState:
    """
        ...
        for tool_name in required_tools:
            ...
            if tool_name == "analyze_company_profile":
                for ticker in companies:
                    ...
                    if company_data:
                        result = tool.invoke({"ticker": ticker, "company_data": company_data})
...
def result_aggregation_node(state: FinancialOrchestratorState) -> FinancialOrchestratorState:
    """
...
def response_planning_node(state: FinancialOrchestratorState) -> FinancialOrchestratorState:
    """
...
def response_generation_routing_node(state: FinancialOrchestratorState) -> FinancialOrchestratorState:
    """
...
def final_response_node(state: FinancialOrchestratorState) -> FinancialOrchestratorState:
    """
    ...
    try:
        ...
        if state.get("needs_clarification", False):
            ...
            response = default_response_generator.generate_clarification_request(clarification_message)
        else:
            # Generate response from analysis results
            response_context_data = state.get("response_context", {})
            response_context = ResponseContext()
            response_context.analysis_results = response_context_data.get("analysis_results", {})
            ...
            response_context.conversation_context = response_context_data.get("conversation_context", {})
        ...
        return update_state_with_agent_response(error_state, f"I encountered an error: {str(e)}")
