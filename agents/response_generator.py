...
class RuleBasedResponseGenerator(BaseResponseGenerator):
    ...
    def _format_analysis_result(self, result_key: str, result: Dict[str, Any], context: ResponseContext) -> Optional[str]:
        """
        ...
        try:
            ...
            elif any(stmt in result_key for stmt in ["income_statement", "balance_sheet", "cash_flow"]):
                statement_name = self._extract_statement_name(result_key)
                return self.display_formatter.format_statement_display(result, statement_name)
...
class HybridResponseGenerator(BaseResponseGenerator):
    ...
    def __init__(self, llm_client=None, use_llm_for_complex: bool = False):
        ...
        self.use_llm_for_complex = use_llm_for_complex and self.llm_generator is not None
        
        logger.info(f"Hybrid response generator initialized (LLM enabled: {self.use_llm_for_complex})")
