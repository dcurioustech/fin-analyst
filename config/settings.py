...
class ConfigurationManager:
    ...
    def _validate_configuration(self, config: Dict[str, Any]) -> None:
        ...
        if (
            config["validation"]["max_ticker_length"]
            < config["validation"]["min_ticker_length"]
        ):
...
def get_app_info() -> Dict[str, str]:
    ...
    """
    return {"name": APP_NAME, "version": APP_VERSION, "description": APP_DESCRIPTION}
...
def load_environment_overrides() -> Dict[str, Any]:
    ...
    if os.getenv("FIN_ANALYST_DEBUG"):
        overrides["debug_mode"] = os.getenv("FIN_ANALYST_DEBUG").lower() in [
            "true",
            "1",
            "yes",
        ]
