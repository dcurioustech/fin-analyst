...
class GCPConfig:
    ...
    def from_environment(cls) -> "GCPConfig":
        ...
        return cls(
            ...
            # Secret Manager
            google_api_key_secret=os.getenv(
                "GOOGLE_API_KEY_SECRET", f"google-api-key-{environment}"
            ),
            alpha_vantage_key_secret=os.getenv(
                "ALPHA_VANTAGE_KEY_SECRET", f"alpha-vantage-key-{environment}"
            ),
            polygon_key_secret=os.getenv(
                "POLYGON_KEY_SECRET", f"polygon-key-{environment}"
            ),
        )
...
class GCPServiceManager:
    ...
    def get_api_keys(self) -> Dict[str, Optional[str]]:
        ...
        if self.config.google_api_key_secret:
            api_keys["google_api_key"] = self.get_secret(
                self.config.google_api_key_secret
            )

        if self.config.alpha_vantage_key_secret:
            api_keys["alpha_vantage_key"] = self.get_secret(
                self.config.alpha_vantage_key_secret
            )
