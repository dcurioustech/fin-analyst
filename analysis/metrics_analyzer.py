...
class MetricsAnalyzer:
    ...
    def analyze_profitability_metrics(self, company_info: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    def analyze_stock_price_metrics(self, company_info: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    def analyze_dividend_metrics(self, company_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            dividend_metrics = {
                ...
                'five_year_avg_dividend_yield': company_info.get('fiveYearAvgDividendYield')
            }
    ...
    def get_comprehensive_metrics(self, company_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            comprehensive_metrics = {
                ...
                'profitability': profitability['data'] if profitability['success'] else {},
        ...
        try:
            ...
            if 'valuation' in metrics and metrics['valuation']:
                ...
                if val_metrics.get('market_cap'):
                    output_lines.append(f"  Market Cap: {format_large_number(val_metrics['market_cap'])}")
                if val_metrics.get('enterprise_value'):
                    output_lines.append(f"  Enterprise Value: {format_large_number(val_metrics['enterprise_value'])}")
                if val_metrics.get('trailing_pe'):
                    output_lines.append(f"  Trailing P/E: {format_ratio(val_metrics['trailing_pe'])}")
                if val_metrics.get('forward_pe'):
                    output_lines.append(f"  Forward P/E: {format_ratio(val_metrics['forward_pe'])}")
                if val_metrics.get('price_to_sales'):
                    output_lines.append(f"  Price to Sales (TTM): {format_ratio(val_metrics['price_to_sales'])}")
                if val_metrics.get('price_to_book'):
                    output_lines.append(f"  Price to Book: {format_ratio(val_metrics['price_to_book'])}")
            ...
            if 'profitability' in metrics and metrics['profitability']:
                ...
                if prof_metrics.get('profit_margins'):
                    output_lines.append(f"  Profit Margins: {format_percentage(prof_metrics['profit_margins'])}")
                if prof_metrics.get('return_on_equity'):
                    output_lines.append(f"  Return on Equity (ROE): {format_percentage(prof_metrics['return_on_equity'])}")
                if prof_metrics.get('return_on_assets'):
                    output_lines.append(f"  Return on Assets (ROA): {format_percentage(prof_metrics['return_on_assets'])}")
                if prof_metrics.get('gross_margins'):
                    output_lines.append(f"  Gross Margins: {format_percentage(prof_metrics['gross_margins'])}")
            ...
            if 'stock_price' in metrics and metrics['stock_price']:
                ...
                if price_metrics.get('current_price'):
                    output_lines.append(f"  Current Price: {price_metrics['current_price']}")
                ...
                if fifty_two_low and fifty_two_high:
                    output_lines.append(f"  52-Week Range: {fifty_two_low} - {fifty_two_high}")
                
                if price_metrics.get('beta'):
                    output_lines.append(f"  Beta: {format_ratio(price_metrics['beta'])}")
            ...
            if 'dividend' in metrics and metrics['dividend']:
                ...
                if div_metrics.get('dividend_yield'):
                    output_lines.append(f"  Dividend Yield: {format_percentage(div_metrics['dividend_yield'])}")
                if div_metrics.get('payout_ratio'):
                    output_lines.append(f"  Payout Ratio: {format_percentage(div_metrics['payout_ratio'])}")
    ...
    def calculate_financial_strength_score(self, metrics: Dict[str, Any]) -> Dict[str, Any]:
        """
