...
class CompanyAnalyzer:
    ...
    def extract_key_company_details(self, company_info: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            'business_summary': company_info.get('longBusinessSummary', 'No summary available.'),
            'market_info': {
    ...
    def validate_company_data(self, company_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        essential_fields = ['longName', 'sector', 'industry', 'currentPrice', 'marketCap']
        missing_essential = []
        ...
        important_fields = ['longBusinessSummary', 'website', 'fullTimeEmployees', 'trailingPE']
        missing_important = []
        ...
        if missing_important:
            validation_results['warnings'].append(f"Missing optional fields: {', '.join(missing_important)}")
        
        # Calculate data quality score (0-100)
        total_fields = len(essential_fields) + len(important_fields)
        available_fields = total_fields - len(missing_essential) - len(missing_important)
        validation_results['data_quality_score'] = int((available_fields / total_fields) * 100)
