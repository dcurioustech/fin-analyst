...
class StatementAnalyzer:
    ...
    def process_income_statement(self, statement_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": True, "data": processed_data, "error": None}
    ...
    def format_statement_for_display(
        self, statement: pd.DataFrame, statement_name: str
    ) -> str:
        """
    ...
    def get_all_statements_analysis(
        self, statements_data: Dict[str, pd.DataFrame]
    ) -> Dict[str, Any]:
        """
            ...
            for metric_name, possible_names in line_items.items():
                for name in possible_names:
                    ...
                    if not matching_rows.empty:
                        ...
                        metrics[metric_name] = income_statement.loc[
                            matching_rows[0]
                        ].iloc[0]
                        break
        ...
        try:
            ...
            for metric_name, possible_names in line_items.items():
                for name in possible_names:
                    ...
                    if not matching_rows.empty:
                        # Get the most recent year (first column)
                        metrics[metric_name] = balance_sheet.loc[matching_rows[0]].iloc[
                            0
                        ]
                        break
        ...
        try:
            ...
            for col in formatted.columns:
                formatted[col] = formatted[col].apply(
                    lambda x: (
                        format_large_number(x) if isinstance(x, (int, float)) else x
                    )
                )
    ...
    def _calculate_cross_statement_metrics(
        ...
        try:
            ...
            if (
                "income_statement" in analysis_results
                and analysis_results["income_statement"]["success"]
            ):
                income_metrics = analysis_results["income_statement"]["data"][
                    "key_metrics"
                ]

            if (
                "balance_sheet" in analysis_results
                and analysis_results["balance_sheet"]["success"]
            ):
                balance_metrics = analysis_results["balance_sheet"]["data"][
                    "key_metrics"
                ]

            if (
                "cash_flow" in analysis_results
                and analysis_results["cash_flow"]["success"]
            ):
